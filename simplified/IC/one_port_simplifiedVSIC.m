...
x=s;
C_f=poly2sym(str2sym('[0 1/m_e]'))/(poly2sym(str2sym('[1 0]')));%+poly2sym(str2sym('[D_t 0]'))/poly2sym(str2sym('[1/tau_d 1]'));
C_m=poly2sym(str2sym('[P_m I_m]'))/(poly2sym(str2sym('[1 0]')));
P=poly2sym(str2sym('[1]'))/(poly2sym(str2sym('[1/omega_0 1]')))
P=eval(P)
C_f=eval(C_f)

Z_c=poly2sym(str2sym('[ K]'))/(poly2sym(str2sym('[1 0]')));%poly2sym(str2sym('[b_f K]'))/(poly2sym(str2sym('[1 0]')))+poly2sym(str2sym('[m_f 0]'))/poly2sym(str2sym('[1/tau_f 1]'));; %m_f b_f
