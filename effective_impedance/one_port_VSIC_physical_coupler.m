...
vars = [F_e,F_f,v_d,v_v,F_v,v_m,F_c,F_int,v_e];
...
x=s;
C_f=poly2sym(str2sym('[0 1/m_e]'))/(poly2sym(str2sym('[1 0]')));
...
Z_c=poly2sym(str2sym('[0 K]'))/(poly2sym(str2sym('[1 0]')))+poly2sym(str2sym('[0 0]'))/poly2sym(str2sym('[1/tau_f 1]'));; %m_f b_fpoly2sym(str2sym('[K]'))*poly2sym(str2sym('[a_0 a_1 1]'))/(poly2sym(str2sym('[1 0]'))*poly2sym(str2sym('[d_0 d_1 1]')));%poly2sym(str2sym('[K]'))/(poly2sym(str2sym('[1 0]')));%
Z_v=eval(Z_v) % transfer function in s domain
Z_d=poly2sym(str2sym('[b_e k_e]'))/(poly2sym(str2sym('[1 0]')))
%poly2sym(str2sym('[0 K*k_e/(K-k_e)]'))/(poly2sym(str2sym('[1 0]')))+poly2sym(str2sym('[0 0]'))/poly2sym(str2sym('[1/tau 1]'));
Z_d=eval(Z_d)
Z_c=poly2sym(str2sym('[K]'))/(poly2sym(str2sym('[1 0]')));
Z_c=eval(Z_c)
Z2=eval(Z_v)
Z3=eval(F_int/sol.v_d)
ss=symvar(eval(Z2));
syms(ss(:),'real')
x=s;
x=s;
...
% syms Z_c C_m P Y_des Z_des
% zc=(Z_c*(C_m*P + Y_des*C_m*P*Z_des + 1))/(Y_des*(Z_des - Z_c - C_m*P*Z_c + C_m*P*Z_des + P*Z_c*Z_des))
% ze=(Z_des*(P*Z_c + C_m*P + Y_des*C_m*P*Z_c + 1))/(Y_des*(Z_des - Z_c - C_m*P*Z_c + C_m*P*Z_des + P*Z_c*Z_des))
% simplify(limit(zc,C_m,inf))
% simplify(limit(ze,C_m,inf))
